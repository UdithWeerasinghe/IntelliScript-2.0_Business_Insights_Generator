...
def generate_other_insights(
    ...
    system = (
        ...
        "Use all the data provided to generate a concise, actionable report."
    )
    ...
    if timeseries_df is not None and forecast_df is not None and category:
        # take last observed + first forecast rows
        last_obs = timeseries_df.iloc[-1]
        first_fc = forecast_df.iloc[0]
        user_parts.append(
            f"## Forecast summary for {category}:\n"
            f"- Last observed {category}: {last_obs.y:.2f} on {last_obs.ds.date()}\n"
            f"- Next period forecast: {first_fc.yhat:.2f} on {first_fc.ds.date()}"
        )
        # attach full table if you like:
        user_parts.append(
            "### Full Forecast Table:\n" +
            forecast_df[['ds','yhat']].to_string(index=False)
        )

    # 3) Then the other texts
    if texts:
        user_parts.append("## Additional extracted text data:")
    ...
    # 5) Call the LLM
    return generate_response_fn(system, full_user)
